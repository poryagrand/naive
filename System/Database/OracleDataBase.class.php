...
/**
 * connect to a oracle database via PDO
 */
class Oracle extends Gate
{

    /**
     * @brief connect to a oracle database . each server can extend this class to connect to its server
     * 
     * @param string $host
     * @param string $dbname
    ...
    function __construct($host, $dbname, $usernam, $password)
    {
        ...
        parent::__construct(
            "oci:dbname=$dbtns;charset=utf8",
            $usernam,
            $password,
            array(
                Gate::ATTR_ERRMODE => Gate::ERRMODE_EXCEPTION,
                Gate::ATTR_EMULATE_PREPARES => false
            )
        );
    ...
    public function create($table, $details, $engine = "", $charset = "", $otherAttributes = [], $columnDetails = [])
    {
        ...
        foreach ($details as $key => $val) {
            ...
            if (isset($val["auto"]) && $val["auto"]) {
                $temp .= " as IDENTITY(START with 1 INCREMENT by 1)";
            }
            if (isset($val["default"])) {
                $temp .= " DEFAULT " . (is_bool($val["default"]) ? ($val["default"] ? 1 : 0) : (is_numeric($val["default"]) ? $val["default"] : ("'" . $val["default"] . "'")));
            }
    ...
    function alterDrop($table, $columns)
    {
        ...
        $query = "ALTER TABLE $table DROP (" . (implode(",", $columns)) . ")";
    ...
    function alterAdd($table, $details)
    {
        ...
        $query = "ALTER TABLE $table ADD ("
            . (implode(", ", $attributes))
            . (empty($primKeys) ? "" : (count($attributes) > 0 ? ", " : "") . " PRIMARY KEY(" . implode(",", $primKeys) . ")") . ")";
    ...
    function alterModify($table, $details)
    {
        ...
        $query = "ALTER TABLE $table MODIFY (" . (implode(", ", $attributes)) . ")";
        return $this->query($query);
    }
    ...
    function alterChnage($table, $details)
    {
        ...
        $query = "ALTER TABLE $table MODIFY (" . $attributes . ")";
